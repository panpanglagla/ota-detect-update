...
boolean Network::connect() {
  //_appSettings.begin("ota-detect-update");
...
boolean Network::startAP() {
  ...
  WiFi.mode(WIFI_AP);
  delay(500);
  if (WiFi.softAP("ota-detect-update", "panpanglagla")) {
...
String Network::renderJS() {
  ...
  str.concat("      const versions = makeElement(\"div\", \"f col g\");\n");
  ...
  str.concat("        versions.appendChild(item);\n");
  str.concat("      }\n");
  str.concat("    }\n");
  str.concat("  }\n");
  str.concat("}\n");
...
String Network::renderHome() {
  ...
  str.concat("      </button>\n");
  str.concat("      <button class=\"radius border border-gray-d red pointer fs-xl b white\" title=\"Check version\" onclick=\"otaCheck();\">\n");
  str.concat("        Check version");
  str.concat("      </button>\n");
  str.concat("    </div>\n");
